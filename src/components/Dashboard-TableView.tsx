  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedDocs = result.documents.map((doc: DocumentResponse) => ({
          id: doc.id,
              ...
              <div className="space-y-4">
                ...
                {verifyModal.verificationData.details?.signatures && verifyModal.verificationData.details.signatures.length > 0 && (
                  <div className="bg-white/5 rounded-lg border border-white/10 p-4">
                    ...
                    {verifyModal.verificationData.details.signatures.map((sig: SignatureVerification, index: number) => (
